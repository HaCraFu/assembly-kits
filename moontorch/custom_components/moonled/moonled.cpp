...
// starting with ESP32 Arduino 2.0.2, the 40MHz crystal is used as clock by
...
inline ledc_mode_t get_speed_mode(uint8_t channel) {
...
float ledc_max_frequency_for_bit_depth(uint8_t bit_depth) {
  return static_cast<float>(CLOCK_FREQUENCY) /
...
float ledc_min_frequency_for_bit_depth(uint8_t bit_depth, bool low_frequency) {
  ...
  return static_cast<float>(CLOCK_FREQUENCY) /
         (max_div_num * static_cast<float>(1 << bit_depth));
}
...
void Moonled::write_state(float state) {
  ...
  if (duty == 1) {
    ...
    pinMode(this->pin_->get_pin(), OUTPUT_OPEN_DRAIN);
...
void Moonled::setup() {
  ...
  chan_conf.duty = this->inverted_ == this->pin_->is_inverted()
                       ? 0
                       : (1U << this->bit_depth_);
...
void Moonled::dump_config() {
  ...
  ESP_LOGCONFIG(TAG,
                ...
                "  Phase angle: %.1f°\n"
                "  Bit depth: %u",
                this->channel_, this->frequency_, this->phase_angle_,
                this->bit_depth_);
...
void Moonled::update_frequency(float frequency) {
  ...
  while (attempt_count_max > 0 && configured_frequency == 0) {
    ESP_LOGV(TAG,
             "Initializing channel %u with frequency %.1f and bit depth of %u",
             this->channel_, this->frequency_, this->bit_depth_);
    configured_frequency =
        ledcSetup(this->channel_, frequency, this->bit_depth_);
    if (configured_frequency != 0) {
      this->initialized_ = true;
      this->status_clear_error();
      ESP_LOGV(TAG, "Configured frequency: %u with bit depth: %u",
               configured_frequency, this->bit_depth_);
    } else {
      ESP_LOGW(TAG,
               "Unable to initialize channel %u with frequency %.1f and bit "
               "depth of %u",
               this->channel_, this->frequency_, this->bit_depth_);
      // try again with a lower bit depth
      this->bit_depth_--;
  ...
  if (configured_frequency == 0) {
    ESP_LOGE(TAG,
             "Permanently failed to initialize channel %u with frequency %.1f "
             "and bit depth of %u",
             this->channel_, this->frequency_, this->bit_depth_);
    this->status_set_error();
    return;
