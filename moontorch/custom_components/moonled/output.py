...
Moonled = ledc_ns.class_("Moonled", output.FloatOutput, cg.Component)
SetFrequencyAction = ledc_ns.class_("SetFrequencyAction", automation.Action)
SetOffAction = ledc_ns.class_("SetOffAction", automation.Action)
SetOnAction = ledc_ns.class_("SetOnAction", automation.Action)

CONFIG_SCHEMA = output.FLOAT_OUTPUT_SCHEMA.extend(
    {
        cv.Required(CONF_ID): cv.declare_id(Moonled),
...
@automation.register_action(
    "output.moonled.set_frequency",
    SetFrequencyAction,
    cv.Schema(
        {
            cv.Required(CONF_ID): cv.use_id(Moonled),
            cv.Required(CONF_FREQUENCY): cv.templatable(validate_frequency),
        }
    ),
)
async def moonled_set_frequency_to_code(config, action_id, template_arg, args):
